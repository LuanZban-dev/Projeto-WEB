...
export class DraftController {
  ...
  async create(request: Request, response: Response) {
    ...
    const player = await draftService.createDraftPlayer(
        request.user.id,
        request.body
    );
  ...
  async index(request: Request, response: Response) {
    ...
    const players = await draftService.listDraftPlayers( request.user.id);
  ...
  async update(request: Request, response: Response) {
    const player = await draftService.updateDraftPlayer(
      request.user.id,
      request.params.id,
      request.body
    );
  ...
  async delete(request: Request, response: Response) {
    await draftService.deleteDraftPlayer(
    request.user.id,
    request.params.id
);
  ...
  async clear(request: Request, response: Response) {
    ...
    await draftService.clearDraft(request.user.id);
